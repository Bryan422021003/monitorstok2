    ...
    <div class="col-md-12">
        <div class="card shadow-sm border-bottom-primary">
            ...
            <div class="card-body">
                ...
                <?= form_open('', [], ['id_barang_keluar' => $brg['id_barang_keluar'], 'user_id' => $this->session->userdata('login_session')['user']]); ?>
                <nav>
                ...
                <div class="tab-content" id="nav-tabContent">
                    <div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab ">
                        <div class="row form-group mt-4">
                            ...
                            <div class="col-md-5">
                                <input value="<?= $brg['id_barang_keluar']; ?>" type="text" readonly="readonly" class="form-control">
                        ...
                        <div class="row form-group">
                            ...
                            <div class="col-md-5">
                                <input value="<?= set_value('tanggal_keluar', $brg['tanggal_keluar']); ?>" name="tanggal_keluar" id="tanggal_keluar" type="text" class="form-control date" placeholder="Tanggal Masuk...">
                                <?= form_error('tanggal_keluar', '<small class="text-danger">', '</small>'); ?>
                            </div>
                        </div>
                        <div class="row form-group">
                            <label class="col-md-2 text-md-right" for="customer_id">Customer</label>
                            <div class="col-md-5">
                                <div class="input-group">
                                    <select name="customer_id" id="customer_id" class="custom-select">
                                        <option value="" selected disabled>Pilih Customer</option>
                                        <?php foreach ($customer as $s) : ?>
                                            <option <?= $brg['customer_id'] == $s['id_customer'] ? 'selected' : ''; ?> <?= set_select('customer_id', $s['id_customer']) ?> value="<?= $s['id_customer'] ?>"><?= $s['nama_customer'] ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                    <div class="input-group-append">
                                        <a class="btn btn-primary" href="<?= base_url('customer/add'); ?>"  target="__BLANK"><i class="fa fa-plus"></i></a>
                                    </div>
                                </div>
                        ...
                        <table class="table w-100 dt-responsive nowrap  " id="">
                            <thead class="thead-light">
                                <tr>
                                    ...
                                    <th>Jumlah</th>
                                    <th>Total</th>
                                    <th colspan="5" class="text-center"><button type="button" onclick="tambah_barang('kurang')" class="btn btn-success"><i class="fas fa-plus"></i></button></th>
                                </tr>
                            </thead>
                            <tbody id="detail_transfer">
                                ...
                                ?>
                                    <tr id="data<?= $i ?>">
                                        ...
                                        <td>
                                            <div class="input-group">
                                                <input type='text' name='jumlah[]' required class='form-control jumlah<?= $i ?> jumlah' value='<?= set_value('jumlah[]', $row['jumlah']); ?>' required onblur='total_masukkeluar(<?= $i ?>,1,"kurang")'>
                                                <div class="input-group-append">
