...
class Stockopname extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['barangkeluar'] = $this->admin->getStockOpNameByCabang($dt_user['cabang_id'], $range);

        $this->form_validation->set_rules('tgl_awal', 'Tanggal Awal', 'required');
        $this->form_validation->set_rules('tgl_akhir', 'Tanggal Akhir', 'required');
        if($this->form_validation->run() == false) {
            $this->template->load('templates/dashboard', 'stock_op_name/data', $data);
        } else {
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $kode = 'S-OP-' . date('ymd');
            $kode_terakhir = $this->admin->getMax('stock_op_name', 'id_stock_op_name', $kode);
            $kode_tambah = substr($kode_terakhir, -5, 5);
            $kode_tambah++;
            $number = str_pad($kode_tambah, 5, '0', STR_PAD_LEFT);
            $data['id_stock_op_name'] = $kode . $number;

            $this->template->load('templates/dashboard', 'stock_op_name/add', $data);
        } else {

            $insert = $this->barang->insertStockOpName();
    ...
    public function edit($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['brg'] = $this->barang->getStockOpNameById($id);

            $d_brg = $this->barang->getDetailStockOpNameById($id);
            ...
            $this->template->load('templates/dashboard', 'stock_op_name/edit', $data);
            
        } else {

            $insert = $this->barang->editStockOpName($id);
    ...
    public function delete($getId)
    {
        ...
        if ($this->barang->delete('stock_op_name', 'id_stock_op_name', $id)) {
    ...
    public function show_stockopname()
    {
        $id_barangkeluar = $this->input->post('id');
        $data['brg'] = $this->barang->getStockOpNameById($id_barangkeluar);
        $data['d_brg'] = $this->barang->getDetailStockOpNameById($id_barangkeluar);

        $this->load->view('stock_op_name/detail_barangkeluar', $data);
    }

    public function cetak_bkluar($id_barangkeluar)
    {
        $brg = $this->barang->getStockOpNameById($id_barangkeluar);
        $d_brg = $this->barang->getDetailStockOpNameById($id_barangkeluar);
